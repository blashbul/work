...
Cette plateforme a pour objectif de fournir une **infrastructure unifiée de gestion du transport** pour un groupe multi-sociétés, couvrant l’ensemble des besoins opérationnels autour du transport de plis et colis.
...
- permettre de passer d’un transport “simple” à une exécution industrialisée (mutualisation, découpage en legs, ruptures de charge, re-routage, délégation carrier) tout en maintenant une promesse commerciale unique (SLA, options, prix) et une traçabilité unifiée.
- intégrer des **sous-traitants internes et externes**,
- évoluer vers des volumes élevés et des organisations complexes.

La plateforme s’inscrit dans une logique **Domain-Driven Design (DDD)**, notamment autour du principe : **transport logique (contractuel) ≠ exécution opérationnelle (terrain)**.  

→ Voir **Principes structurants / Transport logique vs exécution opérationnelle**.
...
- Pricing
- Optimisation (via service externe dédié + orchestration monolithe)
...
- les choix opérationnels (mutualisation, sous-traitance, re-routage) n’impactent pas le contrat tant que les engagements sont respectés.

---

### Options & services additionnels (transport logique)

Lors du passage d’une commande de transport, le client peut sélectionner des  **options** (ou *services additionnels*).
...
L’annonce de colis permet au client de définir :
...
- peuvent déclencher des scénarios de notification (RDV, PIN, etc.).  
→ Voir **Découpage fonctionnel / Notifications**.

---

### Annonce de colis

L’**annonce de colis** correspond à la déclaration préalable d’un ou plusieurs envois par un client ou un système tiers.
...
Une annonce de colis appartient toujours au **transport logique**, indépendamment de la manière dont l’envoi sera exécuté.

---

### Étiquetage

L’**étiquette** est le support physique ou logique permettant d’assurer la **traçabilité opérationnelle** d’un colis.
...
- ou confiés à des **transporteurs externes** pour certains segments.
...
Même en l’absence d’annonce préalable, chaque colis doit rester **traçable** (événements, scans, POD).
La tournée s’appuie généralement sur un mécanisme d’**étiquetage** (y compris réutilisable).  
→ Voir **Annonce de colis & étiquetage**.
...
- délégation de certains legs à des **carriers externes** (DPD, UPS, etc.) en mode *black-box* :
  - remise au carrier (handover), références transporteur,
  - suivi d’exécution via événements (statuts, POD),
  - indépendance vis-à-vis du modèle interne du carrier.
...
- envois **asynchrones**, tolérants aux retards et aux retries,
- idempotence / anti-doublon,
- observable et découplé du flux métier principal.
...
L’optimisation (mix courses “au fil de l’eau” + tournées connues à l’avance) est assurée par :

1) une **orchestration dans le monolithe** (voir BC *Optimization Orchestration*), et  
2) un **service d’optimisation externe**, qui tourne en continu.
...
Les options et contraintes issues du transport logique sont transmises au service d’optimisation sous forme de contraintes planifiables :
...
L’optimiseur ne modifie jamais les options : il cherche une exécution optimale **sous contraintes**.
...
L’intégration repose sur un échange de messages **compatibles Wolverine** côté .NET et consommables côté Java.
